...
func (handler *GenTestHandler) process(ctx *gin.Context, cfg *config.GateWayConfig, checker *ContentChecker, scriptBackend *ScriptBackend) {
	lang := ctx.Param("lang")
	...
	matchProviderIndex, done := checkContent(ctx, cfg.Provider.CodingProviderList, CODE_FUNC_GEN_TEST, lang, content, checker)
	if done {
		return
	}
	...
	if strings.HasPrefix(provider.Backend, "script://") {
		...
		contentList, err := script.CallGenTest(lang, content)
		if err != nil {
			utils.SendError(ctx, 500, err)
			return
		}
